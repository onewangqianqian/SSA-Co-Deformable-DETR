...
class SSA_Adapt(nn.Module):
    def __init__(self, in_planes, out_planes,stride,kernel_size,padding,bias):
        super().__init__()
        self.in_planes = in_planes
        self.out_planes = out_planes
        self.kernel_size = kernel_size
        self.padding = padding
        self.bias = bias
        self.DynamicConv0=DynamicConv(in_planes=in_planes,out_planes=out_planes,kernel_size=kernel_size,stride=stride,padding=padding,bias=False)
        self.DynamicConv1 = DynamicConv(in_planes=in_planes, out_planes=out_planes, kernel_size=kernel_size, stride=stride, padding=padding, bias=False)
        self.DynamicConv2 = DynamicConv(in_planes=in_planes, out_planes=out_planes, kernel_size=kernel_size, stride=stride, padding=padding, bias=False)
        self.DynamicConv3 = DynamicConv(in_planes=in_planes, out_planes=out_planes, kernel_size=kernel_size, stride=stride, padding=padding, bias=False)
    def forward(self, x):
        num_conv = len(x)
        for lvl in range(num_conv):
            x[lvl] = getattr(self, f"DynamicConv{lvl}")(x[lvl])
