...
class CoDeformDETRHead(DETRHead):
    def __init__(self,
        ...
        super(CoDeformDETRHead, self).__init__(
            *args, transformer=transformer, **kwargs) #此处继承了detr_head所以在detr_head中bulid transform
    ...
    def forward_train(self,
        ...
        outs = self(x, img_metas) #既有memory 也有decoder后出来的预测特征
        if gt_labels is None:
            loss_inputs = outs + (gt_bboxes, img_metas)
        else:
            loss_inputs = outs + (gt_bboxes, gt_labels, img_metas)
    ...
    def loss_single(self,
        ...
        cls_avg_factor = max(cls_avg_factor, 1)
        loss_cls = self.loss_cls(
            cls_scores, labels, label_weights, avg_factor=cls_avg_factor) # 分类损失
        ...
        # regression IoU loss, defaultly GIoU loss
        loss_iou = self.loss_iou(
            bboxes, bboxes_gt, bbox_weights, avg_factor=num_total_pos) #iou损失

        # regression L1 loss
        loss_bbox = self.loss_bbox(
            bbox_preds, bbox_targets, bbox_weights, avg_factor=num_total_pos) # bbox损失
